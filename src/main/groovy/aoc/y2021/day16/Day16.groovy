...
class Day16 {
    static final Closure<Boolean> DEFAULT_STOP = { StringBuffer remaining, count -> remaining.length() == 0 || !remaining.toString().contains('1') }
    ...
    static List<Long> readPacket(StringBuffer input, Closure checkVersion, Closure stopCondition = DEFAULT_STOP) {
        List<Long> results = []
        int count = 0
        while (true) {
            count++
            ...
            if (type != 4) {
                if (read(input, 1) == '1') {
                    ...
                    subResults = readPacket(new StringBuffer(read(input, Integer.parseInt(read(input, 15), 2))), checkVersion, DEFAULT_STOP)
                }
            }
            switch (type) {
                case 0:
